...
class DataHouse with ChangeNotifier {
  ...
  initData() async {
    ...
    if (_bhavcopy.isEmpty) {
      final DataSnapshot snapshot = await database.ref().child('bhavcopy').get();
      if(snapshot!.value != null) {
        _bhavcopy = snapshot!.value as Map<dynamic, dynamic>;
    ...
    if (_meta.isEmpty) {
      final DataSnapshot snapshot = await database.ref().child('meta').get();
      if(snapshot!.value != null) {
        _meta = snapshot!.value as Map<dynamic, dynamic>;
